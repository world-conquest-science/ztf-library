// This file is auto-generated by @hey-api/openapi-ts

/**
 * Construct a type with the properties of T except for those in type K.
 */
export type Omit_TBook_formats_ = Pick_TBook_Exclude_keyofTBook_formats__;

/**
 * From T, pick a set of properties whose keys are in the union K
 */
export type Pick_TAuthor_name_or_photo_url_or_about_or_slug_ = {
  slug: string;
  name: string;
  photo_url: string;
  about: string;
};

/**
 * From T, pick a set of properties whose keys are in the union K
 */
export type Pick_TBook_Exclude_keyofTBook_formats__ = {
  slug: string;
  title: string;
  thumbnail: string;
  id: string;
  description: string;
  audio_version_url: string;
  dimensions: string;
  ebook_version_url: string;
  isbn: string;
  pages_count: number;
  publish_date: number;
  publisher: string;
  author: TAuthor;
  categories: Array<TCategory>;
  language: TLanguage;
  image_url: string;
  preface: string;
};

/**
 * From T, pick a set of properties whose keys are in the union K
 */
export type Pick_TBook_slug_or_title_or_thumbnail_ = {
  slug: string;
  title: string;
  thumbnail: string;
};

export type TApiDataReponse_TBook_ = {
  data: TBook;
};

export type TApiDataReponse_TCategory_ = {
  data: TCategory;
};

export type TApiDataReponse_TCategory_Array_ = {
  data: Array<TCategory>;
};

export type TApiDataReponse_TQuote_Array_ = {
  data: Array<TQuote>;
};

export type TApiPaginatedReponse_TBook_Array_ = {
  offset: number;
  limit: number;
  count: number;
  data: Array<TBook>;
};

export type TAuthor = {
  photo_url: string;
  about: string;
  slug: string;
  name: string;
  id: string;
};

export type TBook = {
  preface: string;
  image_url: string;
  thumbnail: string;
  formats: Array<TFormat>;
  language: TLanguage;
  categories: Array<TCategory>;
  author: TAuthor;
  publisher: string;
  publish_date: number;
  pages_count: number;
  isbn: string;
  ebook_version_url: string;
  dimensions: string;
  audio_version_url: string;
  slug: string;
  description: string;
  title: string;
  id: string;
};

export type TCategory = {
  books?: Array<TBook>;
  rank: number;
  description: string;
  title: string;
  slug: string;
  id: string;
};

export type TCurrency = "eur" | "usd" | "xof";

export type TFormat = {
  book?: Omit_TBook_formats_;
  book_id: string;
  prices: Array<TPrice>;
  is_available: boolean;
  label: string;
};

export type TLanguage = {
  code: string;
  id: string;
};

export type TPrice = {
  max_quantity?: number;
  min_quantity?: number;
  currency: TCurrency;
  amount: number;
  id: string;
};

export type TQuote = {
  author: Pick_TAuthor_name_or_photo_url_or_about_or_slug_;
  book: Pick_TBook_slug_or_title_or_thumbnail_;
  content: string;
  id: string;
};

export type GetQuotesResponse = TApiDataReponse_TQuote_Array_ | void;

export type GetQuotesError = unknown;

export type GetRandomQuoteResponse = TApiDataReponse_TQuote_Array_ | void;

export type GetRandomQuoteError = unknown;

export type GetCategoriesData = {
  query?: {
    includeProducts?: boolean;
  };
};

export type GetCategoriesResponse = TApiDataReponse_TCategory_Array_ | void;

export type GetCategoriesError = unknown;

export type GetCategoryBySlugData = {
  path: {
    slug: string;
  };
};

export type GetCategoryBySlugResponse = TApiDataReponse_TCategory_ | void;

export type GetCategoryBySlugError = unknown;

export type GetBooksData = {
  query?: {
    limit?: number;
    offset?: number;
  };
};

export type GetBooksResponse = TApiPaginatedReponse_TBook_Array_ | void;

export type GetBooksError = unknown;

export type GetBooksByCategoryData = {
  path: {
    category_id: string;
  };
  query?: {
    limit?: number;
    offset?: number;
  };
};

export type GetBooksByCategoryResponse =
  TApiPaginatedReponse_TBook_Array_ | void;

export type GetBooksByCategoryError = unknown;

export type GetBookBySlugData = {
  path: {
    slug: string;
  };
};

export type GetBookBySlugResponse = TApiDataReponse_TBook_ | void;

export type GetBookBySlugError = unknown;

export type GetRecommendationResponse = string | void;

export type GetRecommendationError = unknown;
